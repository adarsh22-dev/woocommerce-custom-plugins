<?php
/**
 * Plugin Name: CISAI WCFM Technical Details
 * Description: Adds a single "Technical Details" TinyMCE visual editor with full table, image, media & preview popup to WCFM product edit page. Saves via AJAX and includes shortcode.
 * Version: 2.6
 * Author: Adarsh Singh
 */
...
/* ============================================================
 * 1️⃣ Add Technical Details Editor to WCFM Product Edit
 * ============================================================ */
add_action('after_wcfm_products_manage_general', function($product_id) {
    $meta_key = 'technical_details';
    ...
    <style>
        ...
        #technical_details_editor { border: 1px solid #ccc; border-radius: 6px; }
    ...
    jQuery(document).ready(function($) {
        ...
        $('#save_' + key).on('click', function(e) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'save_wcfm_technical_details',
                    product_id: '<?php echo esc_js($product_id); ?>',
                    content: content,
                    security: '<?php echo wp_create_nonce('save_wcfm_technical_details_nonce'); ?>'
                },
 ...
 * ============================================================ */
add_action('wp_ajax_save_wcfm_technical_details', function() {
    check_ajax_referer('save_wcfm_technical_details_nonce', 'security');
    $product_id = intval($_POST['product_id']);
    $content = wp_kses_post($_POST['content']);
    update_post_meta($product_id, '_technical_details', $content);
    wp_send_json_success(['message' => '✅ Technical Details saved successfully!']);
});
...
/* ============================================================
 * 4️⃣ Shortcode: [product_technical_details id="123"]
 * ============================================================ */
add_shortcode('product_technical_details', function($atts) {
    $atts = shortcode_atts(['id' => get_the_ID()], $atts, 'product_technical_details');

    // Try to get content normally first
    $content = get_post_meta($atts['id'], '_technical_details', true);

    // If it looks like an ACF field key (starts with "field_"), get actual value using ACF
    if (function_exists('get_field') && strpos($content, 'field_') === 0) {
        $content = get_field('technical_details', $atts['id']);
    }

    if (empty(trim($content))) return '';

    return '<div class="product-technical-details" style="margin:30px 0;">' . wp_kses_post($content) . '</div>';
});
